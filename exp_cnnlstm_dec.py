...
from models.dec import CNN_LSTM, DEC, RES_LSTM
from train.train import train_step, test_step
...
cnn_feature_dim = 512  # 使用ResNet提取的特征维度
...
cnn_lstm = RES_LSTM(cnn_output_size=512, hidden_size=128, num_layers=3).cuda()
dec_model = DEC(input_dim=128, n_clusters=10).cuda()

train_step(cnn_lstm, dec_model, dataloader, num_epochs, n_clusters=20)
...
video_summaries = []
with torch.no_grad():
    q, p = test_step(cnn_lstm, dec_model, dataloader)

all_label = np.array(all_label)

video_summaries = np.array(video_summaries)
np.savez_compressed('./saved_summaries/006.npz', arr1 = q, arr2 = p, arr3 = all_label)
