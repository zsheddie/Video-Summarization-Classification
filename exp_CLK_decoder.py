...
from models.CLK_decoder import CNN_LSTM, CLK
from train.train_CLK_decoder import train_step, test_step
...
class RES_VideoDataset(Dataset):
    def __init__(self, video_folder, max_frames=220, resize_shape=(112, 112)):
...
class VideoDataset(Dataset):
    ...
    def _extract_frames(self, video_path):
        ...
        frames = [frames[i] for i in [10,60,110,160,210]]
        return torch.stack(frames).to(self.device)
...
model = CNN_LSTM(cnn_out_dim, lstm_hidden_dim, num_layers).to(device)
...
np.savez_compressed('./saved_summaries/CLK_decoder_002.npz', arr1 = pred, arr2 = all_label)
